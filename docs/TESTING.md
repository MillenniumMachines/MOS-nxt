# nxt Testing Workflow

This document outlines the process for testing new builds of nxt on a live machine using the DuetWebControl (DWC) API directly via `curl`.
...
*   The response is inspected for any error messages generated by RepRapFirmware or custom error messages from nxt macros.
...
```
*   **Verification:** The output of this command must be parsed to check for strings like `Error:`, `Warning:`, or specific success/failure messages from nxt macros.
