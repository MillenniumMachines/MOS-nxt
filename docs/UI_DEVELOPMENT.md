...
## Overview
This guide explains how to develop the nxt UI plugin within the DuetWebControl workspace. The nxt UI is a Vue 2.7 plugin that integrates with Duet Web Control **v3.6.3** (pin: `ci/dwc-build-ref`; shipped ZIPs set exact `dwcVersion` at build) to provide CNC-specific functionality for RepRapFirmware.
...
2. DWC on the machine must match the ZIP’s **`plugin.json` `dwcVersion` exactly** (e.g. `3.6.3`, not merely “3.6.x”). Run `./dist/verify-dwc-build-alignment.sh` before `./dist/build-plugin.sh`. See [PLUGIN_LOAD_TROUBLESHOOTING.md](PLUGIN_LOAD_TROUBLESHOOTING.md).
