# Tool changing in nxt

This note ties together **RepRapFirmware (RRF) stock tool change behaviour**, **nxt’s tooling macros**, and the **optional magazine / ATC extension** (firmware globals + macros). The **nxt DWC plugin Tool Library** lists only **configured RRF tools**; **magazine / job-sequence / ATC operator UI** is intended to live in the **mos-atc** DWC plugin when that pack is installed—not in base nxt.
...
nxt ships example tooling hooks under:
...
`ui/src/utils/nxtToolChangerOm.ts` — export **`NxtToolChangerOmKeys`** maps stable nxt-side names to the actual RRF global strings (unchanged for compatibility with installed firmware).

**M-code numbers** for that extension (used by **mos-atc** UI, not base nxt Tool Library):  
...
| Area | Path |
|------|------|
| nxt default globals | `macros/system/nxt-vars.g`, `macros/system/nxt-tooltable.g` (mosTT) |
