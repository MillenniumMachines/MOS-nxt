...
var toolSame = { global.nxtTT[param.P] != null && #tools > param.P && tools[param.P] != null }

; Radius + spindle match
set var.toolSame = { var.toolSame && global.nxtTT[param.P][0] == param.R && tools[param.P].spindle == var.spinId }
...
if { exists(param.X) }
    set var.toolSame = { var.toolSame && global.nxtTT[param.P][1][0] == param.X }
if { exists(param.Y) }
    set var.toolSame = { var.toolSame && global.nxtTT[param.P][1][1] == param.Y }

; Optional flute count (F) / flute length (L) match - stored in nxtTT[P][2],[3]; -1 = unset
if { exists(param.F) }
    if { #global.nxtTT[param.P] > 2 }
        set var.toolSame = { var.toolSame && global.nxtTT[param.P][2] == param.F }
    else
        set var.toolSame = false
if { exists(param.L) }
    if { #global.nxtTT[param.P] > 3 }
