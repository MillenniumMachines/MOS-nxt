...
if { var.dXAngleDiff > global.nxtProbeAngleTol }
    abort { "Rectangular block surfaces on X axis are not parallel (" ^ var.dXAngleDiff ^ " > " ^ global.nxtProbeAngleTol ^ ") - this block does not appear to be square." }
...
if { var.dYAngleDiff > global.nxtProbeAngleTol }
    abort { "Rectangular block surfaces on Y axis are not parallel (" ^ var.dYAngleDiff ^ " > " ^ global.nxtProbeAngleTol ^ ") - this block does not appear to be square." }
...
if { var.cornerAngleError > global.nxtProbeAngleTol }
    abort { "Rectangular block corner angle is not perpendicular (" ^ var.cornerAngleError ^ " > " ^ global.nxtProbeAngleTol ^ ") - this block does not appear to be square." }
...
set global.nxtWPDims[var.workOffset][0] = { abs(((var.pSfcX[0][0][0][0] + var.pSfcX[0][0][1][0]) / 2) - ((var.pSfcX[1][0][0][0] + var.pSfcX[1][0][1][0]) / 2)) }
set global.nxtWPDims[var.workOffset][1] = { abs(((var.pSfcY[0][0][0][1] + var.pSfcY[0][0][1][1]) / 2) - ((var.pSfcY[1][0][0][1] + var.pSfcY[1][0][1][1]) / 2)) }
...
if { !exists(param.R) || param.R != 0 }
    ...
    echo { "nxt: Setting WCS " ^ var.wcsNumber ^ " X,Y origin to the center of the rectangle block." }
