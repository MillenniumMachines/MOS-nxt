...
- **Numbered G/M via M98 (hard stop):** Do not `M98 P"M….g"` / `M98 P"G….g"`. Files on `0:/sys/` are meta commands — call `M6520` / `G6503` directly (`M98` steals `P`). Named helpers are OK without a nested **`P`**. Run `node dist/check-m98-numbered-meta.mjs`. See [`.cursor/rules/rrf-numbered-meta-m98.mdc`](../.cursor/rules/rrf-numbered-meta-m98.mdc) and [`docs/RRF_META_PITFALLS.md`](../docs/RRF_META_PITFALLS.md).
