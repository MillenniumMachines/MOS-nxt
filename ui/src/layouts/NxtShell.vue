<!--
  NxtShell — DWC custom layout (registerLayout id "nxt").
  Adapted from DuetWebControl src/layouts/builtin.vue; re-diff when bumping ci/dwc-build-ref.
  Only intentional delta vs stock: CNC status slot uses nxt CNCContainerPanel (tool/WCS/offset).
-->
<style scoped>
.machine-name {
	min-width: 0;
...
import {
	...
	NavMenuBadge,
	UploadButton,
} from "DuetWebControl/components";
...
import NxtCncContainerPanel from "../components/overrides/panels/CNCContainerPanel.vue";

// Host Vuetify's useDisplay() is not externalised for plugins — mirror default breakpoints.
function useNxtDisplayBreakpoints() {
	const smAndUp = ref(false);
	const mdAndUp = ref(false);
	const lgAndUp = ref(false);
	const breakpointName = ref("xs");

	function sync() {
		const w = window.innerWidth;
		smAndUp.value = w >= 600;
		mdAndUp.value = w >= 960;
		lgAndUp.value = w >= 1280;
		if (w < 600) {
			breakpointName.value = "xs";
		} else if (w < 960) {
			breakpointName.value = "sm";
		} else if (w < 1280) {
			breakpointName.value = "md";
		} else if (w < 1920) {
			breakpointName.value = "lg";
		} else {
			breakpointName.value = "xl";
		}
	}
	...
	return { smAndUp, mdAndUp, lgAndUp, name: breakpointName };
}
...
const { mdAndUp, lgAndUp, smAndUp, name: breakpointName } = useNxtDisplayBreakpoints();
