...
export function runAllNxtUserVarsPersistenceTests(): void {
  ...
  if (!deflScalar || deflScalar.join(',') !== '0.025,0.025') {
    throw new Error('readConfigDeflectionXY scalar normalize failed')
  }
  const deflVec = readConfigDeflectionXY({ 0: 0.01, 1: 0.02 })
  if (!deflVec || deflVec.join(',') !== '0.01,0.02') {
    throw new Error('readConfigDeflectionXY vector failed')
  }
  ...
  const snap = snapshotConfigFromOm({
    ...
    nxtProbeDeflection: { 0: 0.01, 1: 0.02 }
  })
  if (!snap.nxtFeatureTouchProbe || snap.nxtSpindleID !== 0) {
    throw new Error('snapshotConfigFromOm failed')
  }
  if (!snap.nxtProbeDeflection || snap.nxtProbeDeflection.join(',') !== '0.01,0.02') {
    throw new Error('snapshotConfigFromOm deflection XY failed')
  }

  const deflGcode = buildNxtUserVarsGcode({
    ...emptyConfigDraft(),
    nxtProbeDeflection: [0.01, 0.02]
  })
  if (!deflGcode.includes('set global.nxtProbeDeflection = {0.01, 0.02}')) {
    throw new Error('buildNxtUserVarsGcode should persist deflection as vector')
  }
  ...
  if (!fromMos.nxtProbeDeflection || fromMos.nxtProbeDeflection.join(',') !== '0.03,0.04') {
