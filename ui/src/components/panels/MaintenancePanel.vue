<template>
  <v-card variant="outlined" class="mt-4">
    <v-card-title class="subtitle-1 d-flex align-center py-3">
    ...
    <v-card-text class="pa-3">
      <v-alert v-if="!maintenanceEnabled" type="info" density="compact" variant="outlined" class="mb-3">
        {{ $t('plugins.nxt.panels.maintenance.disabledHint') }}
      </v-alert>

      <div class="text-subtitle-2 mb-2">{{ $t('plugins.nxt.panels.maintenance.axisTravel') }}</div>
            ...
            <tr v-for="row in axisRows" :key="'ax' + row.index">
              ...
              <td>
                <v-text-field
                  ...
                  :disabled="uiFrozen"
                  @change="setAxisThreshold(row.index, $event.target.value)"
                />
              </td>
              <td>
                <v-progress-linear
      ...
      <template v-if="coolantEnabled">
        ...
        <div class="d-flex align-center mb-2">
          <v-icon class="mr-2" size="small">mdi-water-outline</v-icon>
          <span class="text-subtitle-2">{{ $t('plugins.nxt.panels.maintenance.coolantRuntime') }}</span>
          <v-spacer />
          <v-chip v-if="coolantDue" size="x-small" label color="warning" text-color="white">
            {{ $t('plugins.nxt.panels.maintenance.serviceDue') }}
          </v-chip>
        </div>
        <v-row density="compact" align="center">
          <v-col cols="12" sm="4">
            <div class="text-body-2">{{ coolantLabel }}</div>
          </v-col>
          <v-col cols="12" sm="4">
            <v-text-field
              :model-value="coolantServiceMins"
              type="number"
              min="0"
              step="1"
              density="compact"
              ...
              :disabled="uiFrozen"
              @change="setCoolantThreshold($event.target.value)"
            />
          </v-col>
          <v-col cols="12" sm="4">
            <v-progress-linear :model-value="coolantPct" :color="coolantDue ? 'warning' : 'primary'" height="8" rounded />
          </v-col>
        </v-row>
        <v-btn
          size="x-small"
      ...
      <v-divider class="my-4" />
      <div class="text-subtitle-2 mb-2">{{ $t('plugins.nxt.panels.maintenance.idleActions') }}</div>
      <v-switch
        ...
        density="compact"
        class="mt-0"
        @update:model-value="setIdleEnabled"
      />
      <v-row density="compact">
        <v-col cols="12" sm="4">
          <v-text-field
            ...
            @change="setIdleMinutes($event.target.value)"
          />
        </v-col>
        <v-col cols="12" sm="4">
          <v-text-field
            :model-value="idleFanPct"
            type="number"
            min="0"
            max="100"
            step="1"
            ...
            :disabled="uiFrozen || !idleEnabled"
            @change="setIdleFanPct($event.target.value)"
          />
        </v-col>
        <v-col cols="12" sm="4">
          <v-text-field
            :model-value="idleDimBri"
            type="number"
            min="0"
            max="255"
            step="1"
            ...
            :disabled="uiFrozen || !idleEnabled"
            @change="setIdleDimBri($event.target.value)"
          />
        </v-col>
      </v-row>
    </v-card-text>
    ...
    <v-dialog v-model="serviceDialog" max-width="400">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn variant="text" @click="serviceDialog = false">{{ $t('plugins.nxt.panels.maintenance.cancel') }}</v-btn>
          <v-btn color="primary" variant="text" :disabled="uiFrozen" @click="doService">
            {{ $t('plugins.nxt.panels.maintenance.serviced') }}
          </v-btn>
        </v-card-actions>
      </v-card>
...
export default defineNxtComponent({
  ...
  methods: {
    ...
    async setAxisThreshold(index: number, raw: string | number) {
      ...
      // M5013 resizes nxtAxisServiceAt if nxt-vars sized it before M584 (#move.axes was 0).
      await this.sendCode('M5013 A' + index + ' S' + mm)
