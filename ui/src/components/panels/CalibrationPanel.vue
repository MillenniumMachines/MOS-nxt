<template>
  <v-card outlined>
    ...
    <v-card-text>
      ...
      <v-alert v-if="!isConnected" type="warning" dense outlined class="mb-3">
        {{ $t('plugins.nxt.panels.calibration.needConnection') }}
      </v-alert>

      <!-- Phase 0: static datum -->
      <v-card v-if="needsProbeDatumSetup" outlined class="mb-4 pa-3">
        <div class="subtitle-2 mb-2">{{ $t('plugins.nxt.panels.calibration.phase0Title') }}</div>
        <p class="text-caption grey--text mb-3">{{ $t('plugins.nxt.panels.calibration.phase0Hint') }}</p>
        <div class="d-flex flex-wrap mb-3">
          <v-chip small :color="toolSetterPosSet ? 'success' : 'warning'" outlined>
            {{ $t('plugins.nxt.panels.calibration.chipToolSetter') }}: {{ toolSetterPosDisplay }}
          </v-chip>
          <v-chip small :color="touchProbeRefPosSet ? 'success' : 'warning'" outlined>
            {{ $t('plugins.nxt.panels.calibration.chipRefPos') }}: {{ touchProbeRefPosDisplay }}
          </v-chip>
          <v-chip small :color="deltaMachineSet ? 'success' : 'warning'" outlined>
      ...
      <v-row v-if="probeModeSelectable" class="mb-3" dense>
        <v-col cols="12" sm="6" md="4">
          <v-btn-toggle v-model="calMode" mandatory dense color="primary" divided>
      ...
      <!-- Rotary A section -->
      <v-card v-if="rotaryAvailable && selectedAxis === 'A'" outlined class="mb-4 pa-3">
        <div class="subtitle-2 mb-2">{{ $t('plugins.nxt.panels.calibration.rotarySection') }}</div>
        <p class="text-caption grey--text mb-3">{{ $t('plugins.nxt.panels.calibration.rotaryHint') }}</p>
        <v-row dense>
          <v-col cols="12" md="4">
            <v-btn block color="primary" outlined :disabled="uiFrozen" @click="runRotary(cmdM4912ProbeYFlatness())">
              {{ $t('plugins.nxt.panels.calibration.runM4912') }}
            </v-btn>
          </v-col>
          <v-col cols="12" md="4">
            <v-btn block color="primary" outlined :disabled="uiFrozen" @click="runRotary(cmdM4910ProbeYCenter())">
          ...
          <v-col cols="12" md="4">
            <v-select
              ...
              class="mb-2"
            />
            <v-btn block color="primary" outlined :disabled="uiFrozen" @click="runRotary(cmdM4807ApplyY0(rotaryWcs))">
        ...
        <v-row dense>
          <v-col cols="6" md="3">
            <v-text-field v-model.number="aCommanded" type="number" step="0.001" dense outlined
              :label="$t('plugins.nxt.panels.calibration.commanded')" hide-details />
          </v-col>
          <v-col cols="6" md="3">
            <v-text-field v-model.number="aActual" type="number" step="0.001" dense outlined
      ...
      <template v-if="calMode === 'probe' && !needsProbeDatumSetup">
        ...
        <v-card outlined class="mb-4 pa-3">
          <div class="subtitle-2 mb-2">{{ $t('plugins.nxt.panels.calibration.probeModeTitle') }}</div>
          <p class="text-caption grey--text mb-3">{{ $t('plugins.nxt.panels.calibration.probeModeHint') }}</p>
      ...
      >
        ...
        <v-expansion-panel v-if="calMode === 'manual'">
          <v-expansion-panel-header>{{ $t('plugins.nxt.panels.calibration.phase1Title') }}</v-expansion-panel-header>
          <v-expansion-panel-content>
            <p class="text-caption mb-2">{{ $t('plugins.nxt.panels.calibration.phase1Hint') }}</p>
            <p class="text-caption grey--text mb-2">{{ $t('plugins.nxt.panels.calibration.residualSignHint') }}</p>
            <v-row dense>
              <v-col cols="12" md="4">
                <v-select
                  v-model="p1FaceAway"
            ...
            <p class="text-caption grey--text mt-2 mb-2">{{ $t('plugins.nxt.panels.calibration.runMotionTestHint') }}</p>
        ...
        <!-- Phase 2 -->
        <v-expansion-panel v-if="calMode === 'manual'">
          <v-expansion-panel-header>{{ $t('plugins.nxt.panels.calibration.phase2Title') }}</v-expansion-panel-header>
          <v-expansion-panel-content>
            ...
            <v-row dense class="mt-2">
              <v-col cols="6" md="3">
                <v-text-field v-model.number="p2Measured1" type="number" step="0.001" dense outlined
                  :label="$t('plugins.nxt.panels.calibration.measured1')" hide-details />
              </v-col>
              <v-col cols="6" md="3">
                <v-text-field v-model.number="p2Measured2" type="number" step="0.001" dense outlined
                  :label="$t('plugins.nxt.panels.calibration.measured2')" hide-details />
              </v-col>
            </v-row>
            <v-row dense class="mt-2">
              <v-col cols="12" md="4">
                <v-text-field v-model.number="probeTarget" type="number" step="0.001" dense outlined
              ...
              <v-col cols="6" md="2">
                <v-btn block outlined :disabled="!isConnected" @click="captureProbeInto('m1')">
                  {{ $t('plugins.nxt.panels.calibration.captureM1') }}
                </v-btn>
              </v-col>
              <v-col cols="6" md="2">
                <v-btn block outlined :disabled="!isConnected" @click="captureProbeInto('m2')">
                  {{ $t('plugins.nxt.panels.calibration.captureM2') }}
                </v-btn>
              </v-col>
            </v-row>
            <p class="text-caption grey--text mt-1">{{ $t('plugins.nxt.panels.calibration.runG6512JogHint') }}</p>
        ...
        <!-- Phase 3 -->
        <v-expansion-panel v-if="calMode === 'manual'">
          <v-expansion-panel-header>{{ $t('plugins.nxt.panels.calibration.phase3Title') }}</v-expansion-panel-header>
          <v-expansion-panel-content>
            <p class="text-caption mb-2">{{ $t('plugins.nxt.panels.calibration.phase3Hint') }}</p>
            <v-row dense>
              <v-col cols="6" md="3">
                <v-text-field v-model.number="blMeanPos" type="number" step="0.001" dense outlined
                  :label="$t('plugins.nxt.panels.calibration.meanPositive')" hide-details />
              </v-col>
              <v-col cols="6" md="3">
                <v-text-field v-model.number="blMeanNeg" type="number" step="0.001" dense outlined
            ...
            <v-row dense class="mt-2">
              <v-col cols="12" md="4">
                <v-btn block outlined small :disabled="!isConnected" @click="addBacklashSample('pos')">
                  {{ $t('plugins.nxt.panels.calibration.addSamplePos') }}
                </v-btn>
              </v-col>
              <v-col cols="12" md="4">
                <v-btn block outlined small :disabled="!isConnected" @click="addBacklashSample('neg')">
        ...
        <!-- Phase 4 -->
        <v-expansion-panel>
          <v-expansion-panel-header>{{ $t('plugins.nxt.panels.calibration.phase4Title') }}</v-expansion-panel-header>
          <v-expansion-panel-content>
            ...
            <v-row dense>
              ...
              <v-col cols="6" md="3">
                <v-text-field v-model.number="defMeasured" type="number" step="0.001" dense outlined
        ...
        <!-- Phase 5 -->
        <v-expansion-panel v-if="calMode === 'manual'">
          <v-expansion-panel-header>{{ $t('plugins.nxt.panels.calibration.phase5Title') }}</v-expansion-panel-header>
          <v-expansion-panel-content>
            <p class="text-caption mb-2">{{ $t('plugins.nxt.panels.calibration.phase5Hint') }}</p>
            <v-btn outlined class="mb-2" :disabled="!isConnected" @click="sendCode('M6523 B0 C10')">
            ...
            <v-alert v-if="statusMsg" :type="statusType" dense outlined class="mt-3">
              {{ statusMsg }}
            </v-alert>
          </v-expansion-panel-content>
        </v-expansion-panel>
