    ...
    <v-card outlined>
      <v-card-title class="subtitle-1 d-flex flex-wrap align-center py-3">
        ...
        <v-btn
          ...
          @click="toggleLock"
        >
          <v-icon left small>{{ locked ? 'mdi-lock' : 'mdi-lock-open-variant' }}</v-icon>
          {{
            locked
      ...
      <v-card-text class="pa-3">
        ...
        <p v-if="locked" class="body-2 warning--text mb-3">
    ...
    <v-dialog v-model="importDialog" max-width="720">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-simple-table v-if="importRows.length > 0" dense class="mt-3">
            <thead>
              <tr>
                <th>T#</th>
